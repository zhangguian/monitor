...
| 采集器类型 | 功能说明 | 核心实现文件 |
|------------|----------|--------------|
| 行为采集器 | 采集用户点击、滚动、路由切换等行为 | behavior.ts |
| 错误采集器 | 采集 JS 运行时错误、Promise 错误、框架错误 | error.ts |
...
5. Worker 接收日志，进行采样、脱敏、去重处理
6. 处理后的日志加入内存队列
7. 当队列长度达到阈值或定时触发时，进行批量上报
8. 上报成功的日志从队列中移除，失败的日志持久化到 IndexedDB
9. 下次初始化时，从 IndexedDB 恢复未上报日志
